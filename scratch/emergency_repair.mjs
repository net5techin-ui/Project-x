
import { createClient } from '@supabase/supabase-js';

const supabaseUrl = 'https://orzhjgrjpxrlikswwenc.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9yemhqZ3JqcHhybGlrc3d3ZW5jIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUyNzIwNzMsImV4cCI6MjA5MDg0ODA3M30.wRTYCtHvkDfLXQTB6rktUUUbu27e4GpzJMwzSXsloUE';
const supabase = createClient(supabaseUrl, supabaseKey);

async function tryRepair() {
  console.log('Attempting to check only IDs...');
  const { data, error } = await supabase.from('products').select('id');
  if (error) {
    console.error('ID Select Error:', error);
    
    console.log('Attempting emergency DELETE ALL to clear corruption...');
    const { error: delError } = await supabase.from('products').delete().neq('id', 0);
    if (delError) console.error('Delete failed:', delError);
    else console.log('✅ Emergency clear successful');
  } else {
    console.log('IDs found:', data.length);
  }
}

tryRepair();
